// Управление пультом: фокус ходит по стрелкам сам.
//
// Почему это нужно. В WebView стрелки пульта не перемещают фокус — они
// прокручивают страницу ровно так же, как на десктопе. Пространственная
// навигация (spatial navigation), которая в Хроме умеет водить фокус по
// геометрии, в WebView отключена. Итог: пультом не попасть ни на один
// постер, хотя каждый из них — настоящая <button> и фокус принять готов.
//
// Что делаем: по нажатию стрелки ищем среди фокусируемых тот элемент, что
// лежит в нужную сторону от нынешнего и ближе всех к нему, и сами переносим
// на него фокус. Считаем по геометрии, а не по порядку в разметке: полки
// идут вбок, и «следующий в DOM» там не значит «справа».
//
// Включается только на слабой платформе: на десктопе стрелками крутят
// страницу, и отбирать у них это незачем.
...
/**
 * Доводит фокус до середины кадра, а у вложенного списка — до середины
 * самой плитки.
 *
 * ПЛИТКА ФРАНШИЗЫ (`[data-hold]`)
 *
 * У неё свой вертикальный просмотр внутри, и обычный `scrollIntoView`
 * делает лишнее: он обходит ВСЕХ прокручиваемых предков, а значит
 * увозит и страницу. Человек листает хронологию из трёх десятков
 * частей, а экран под ним каждый шаг скачет на полкорпуса.
 *
 * Страницу при этом гасить нельзя вовсе: плитка стоит внизу карточки,
 * и без доводки она уезжает за край — выйти из неё вниз можно, а
 * вернуться обратно уже нет.
 *
 * Поэтому делим работу: список крутим сами (выбранная строка приходит
 * к середине плитки), а страницу доводим до середины самой плитки.
 * Ось плитки совпадает с осью экрана, и она стоит на месте, сколько
 * внутри ни листай.
 *
 * ВСЁ ОСТАЛЬНОЕ
 *
 * Своего списка нет — прокрутка остаётся браузеру. Так было всегда,
 * и это правильно: полки главной прокручивает документ, и обходить там
 * нечего.
 */
...
/**
 * Ставит фокус на первый элемент содержимого.
 *
 * Зачем. Пока фокус ни на ком, стрелки ведёт сама оболочка — своим порядком,
 * первым по разметке, а это пункт рельса слева. Наше нажатие в этом
 * состоянии до обработчика не доходит вовсе: оболочка съедает его раньше
 * (замер на приставке: первое нажатие после запуска и после смены экрана
 * в журнале пульта не появляется, фокус при этом уже в рельсе). Значит,
 * первое нажатие нельзя перехватить — его надо не допустить: фокус обязан
 * уже стоять в содержимом до того, как человек дотронется до пульта.
 *
 * Прокрутку не трогаем: фокус ставится на то, что и так видно, а доводка
 * кадра — дело хода стрелкой.
 *
 * Исход: `ok` — фокус встал, `wait` — сеять ещё некуда (экран не нарисован),
 * `fail` — цель была, но фокус не приняла.
 *
 * Различать `wait` и `fail` необходимо: пульт подключается до createApp(),
 * и первые такты сторожа приходятся на пустой документ. Сосчитать их как
 * неудачи — значит исчерпать запас попыток за две секунды и остаться
 * без фокуса на экране, который рисовался дольше.
 */
export function seedDpad(): Seed {
  const area = scope()
