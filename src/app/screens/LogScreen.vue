...
/** Виды записей для отбора. Порядок по частоте вопроса: сначала «что сломалось»,
 * потом «что происходило». */
const KINDS: ReadonlyArray<LogType> = ['ERROR', 'WARN', 'INFO', 'API', 'DB']

/** Сколько строк рисуем разом. Буфер вмещает пятьсот, но показ всех сразу на
 * приставке заметен глазом, а читают всегда свежие. */
const PAGE = 120
...
/** Подписка на поток. Перерисовка идёт целиком, а не вставкой одной строки: отбор и
 * потолок показа всё равно считаются по всему буферу, а записей в секунду единицы. */
...
/** Панель бюджета складная и опрашивается только открытой. Снимок дешёвый, но опрос
 * ради закрытой панели — та же расточительность, только обращённая внутрь программы. */
...
/** Склад считается по кнопке, а не сам собой: getDbStats обходит все ключи mediaCache,
 * а их бывает под сотню тысяч. Открытый экран не должен стоить полного обхода базы в секунду. */
...
/** Журнал в буфер обмена: человеку проще прислать текст, чем описывать словами.
 * Уходит то, что видно на экране, вместе с отбором. */
...
onBeforeUnmount(() => {
// Снимать обязательно: иначе подписка переживёт экран и будет дёргать перерисовку выброшенных строк.
...
<style scoped>
/* Шапка, счётчик и две кнопки в одной полосе: три строки подряд съедали первый экран ради трёх подписей. */
.am-log__top {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
...
/* Две кнопки счётчиков стоят своей полосой, а не в шапке: там уже живут действия над самим журналом. */
.am-log__tools {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
...
/* Склад — не таблица: пар «что» и «сколько» много, а колонок две, и на узком окне сетка ложится в столбец сама. */
.am-log__store {
  display: grid;
...
/* Строка — не панель со стеклом: сто двадцать размытий за кадр видно глазом. Цвет вида записи живёт одним --am-tint. */
.am-log__row {
...
/* Подробности переносятся: строка запроса длиннее окна, а горизонтальная прокрутка внутри списка ломает чтение. */
.am-log__body {
  margin: 8px 0 0;
  padding: 10px 12px;
  font-size: 12px;
