...
export function useMediaCard(mediaId: Ref<number>): MediaCardView {
  ...
  const aboutLinks = computed<MediaLink[]>(() => {
    // На телевизоре браузера нет, и весь хвост — сплошные переходы
    // наружу. Не рисуем его вовсе: ряд ссылок, которые никуда не ведут,
    // только занимает место под описанием.
  ...
  /**
   * Прокручивает полку франшизы к нынешнему аниме — и только её.
   *
   * Прежде здесь стоял `scrollIntoView`, и он доводил до видимости не одну
   * полку, а всех прокручиваемых родителей: страницу карточки в том числе.
   * Оттого карточка при открытии уезжала с баннера к полке франшизы —
   * ровно к тому месту, куда человек ещё не собирался. Теперь двигается
   * только `scrollLeft` самой полки, вертикальное положение страницы
   * не трогаем вовсе.
   */
