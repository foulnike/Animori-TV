...
// Главная — витрина рекомендаций (пункт 3.11). Своя полка из памяти
// коллекции, витрина каталога через core/recs: сети экран не знает.
//
// Три полки каталога едут одним запросом (packShelf: AniList умеет
// несколько страниц под разными именами сразу). Пока отбор пуст —
// витрина из пяти полок; выбран жанр, тэг, год или формат — каруселей
// нет вовсе, вместо них вертикальная лента с «Показать ещё». Порция
// ленты — целое число рядов сетки, иначе нижний ряд стоит недобранным.
