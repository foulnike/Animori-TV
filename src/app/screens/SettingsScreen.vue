...
//
// Чего здесь нет на приставке и почему: вход в AniList (нужен браузер),
// выгрузка в XML (нужен проводник), плашка со звездой (ведёт на GitHub —
// см. canOpenOutside). Шикимори осталась: вход ей не нужен, а ник пультом
// набрать можно. Список на приставку приходит копией по ссылке.
//
// Панели собраны в колонки-обёртки, а не в общие строки сетки: высокая
// панель облака иначе держала строку и под соседними зияла пустота. Число
// колонок под каждый размер окна — в settings-screen.css; на приставке
// колонка ровно одна. Узлы свои: ProxyBox, CloudBox, BrandMark.
 ...
 * Исключение — полки витрины: состав собран заранее, поэтому тумблер выбрасывает его через forgetRecs.
 *
 * Включение спрашивает дату рождения: проверка формальная, дата не уходит ни в настройки, ни на склад.
 * Тумблер встаёт в «включено» только после ответа.
 */
