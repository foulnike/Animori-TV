...
/** Сколько записей рисуется за раз и на сколько растёт потолок при доборе. Полный список
 * бывает на тысячи записей, а каждая требует обложки и русского названия. */
...
/** Виды показа. Плитки хороши на десятках записей, а в закладке на тысячу строка
 * видна целиком и читается быстрее постера. */
...
// Закладка, порядок, вид и слово берутся из памяти между показами: иначе возврат с карточки открывал чужую закладку.
...
/** Сколько строк показываем сейчас. Растёт шагом PAGE_LIMIT по добору, а на любой
 * смене отбора возвращается к первой сотне. */
const limit = ref(PAGE_LIMIT)

/** Сколько строк отобралось до обрезки потолком. */
const picked = ref(0)

/** Сколько строк спрятал тумблер показа взрослого. Нужно подписи под списком: закладка
 * на сорок три записи, где видно сорок, читалась бы потерей своих же данных. */
const hidden = ref(0)
...
/** Закладка записи для окна правки. У записи в памяти закладки может не быть: снимок
 * допускает пустоту. Подставлять сюда открытую закладку списка нельзя — окно соврало бы. */
...
/** Найденные записи последнего поиска. Не реактивные сознательно: по ним
 * перерисовываются плитки, когда добрались обложки и названия. */
...
/** Снимает кусок коллекции в свои плитки. При идущем поиске плитки берутся из находок:
 * закладка тогда не главная, а слово главное. Потолок отрезается после сортировки. */
function redraw(): void {
// Взрослое прячется и здесь: тумблер один на всё приложение. Числа считаются по видимому,
// а не по всей коллекции: подпись «Закладки: 43» над сорока строками читалась бы пропажей.
...
// Доборы обложек, названий и меток живут рядом со сборкой строки: экран отдаёт свои строки и перерисовку.
...
/** Отбор по слову по всем закладкам сразу. Сети не требует, но на кириллице
 * сначала поднимает склад русских названий. */
...
/** Смена порядка со своего ролл-аута. Ключ приходит строкой, поэтому сверяем
 * с известными: чужое значение в память порядка попасть не должно. */
...
/** Смена вида показа. Потолок показа не сбрасывается: записи те же самые, меняется только
 * одежда. Метки доступности не заказываются: переход на постеры покажет плитки, и вопрос задаст директива. */
...
onMounted(() => {
// Смотритель за концом списка: добор заказывается заранее. На старых окружениях смотрителя может не быть — останется кнопка.
  ...
  void (async () => {
    try {
// Только снимок с диска: в сеть за списком экран сам не ходит. Перенос человек заводит в настройках.
...
<!-- Порядок показа на своём ролл-ауте: системный список выпадал белым на тёмных темах — его рисует оболочка. -->
      <PickBox
        class="am-sort"
        :model-value="sortKey"
        :items="SORT_TABS"
...
<!-- Заглушки под тот вид, который человек выбрал: ряды заглушек держат ту же высоту строки, что и
     настоящие, и экран не дёргается в момент подъёма списка. -->
    <ul
...
<!-- Пустой список и список, целиком скрытый меткой 18+, — разные вещи: первое правда, второе ложь. -->
...
<!-- Пустая закладка — не пустой список: записи у человека есть, они просто в других закладках.
     Прежде здесь стояла одна строка «записей нет», и она читалась как потеря списка целиком. -->
...
<!-- Метки показа на строках нет нарочно: её вешали на плитки, потому что доступность источника
     показывает только постер. Строка метку не рисует, и заказ был бы вопросом ради невидимого знака. -->
...
<!-- То же окно, что на карточке. Признак онгоинга и потолок счёта берутся из склада обликов:
     если облик ещё не доехал, потолка нет и прыжка к концу счёта тоже — это штатно. -->
    <EntrySheet
      v-if="editRow"
      :title="editName"
      :status="editStatus"
...
/* Выбор порядка — свой ролл-аут: вся отделка в самом компоненте, здесь только место в ряду и потолок ширины. */
.am-sort {
  flex: 0 1 220px;
  max-width: 100%;
}

/* Вид показа — три значка в одной капсуле. Подписи ушли в подсказку: «Большие постеры» занимали половину полосы. */
.am-look {
  flex: 0 0 auto;
}

/* Знак ставится сеткой в кнопку известной ширины: на padding он гулял по кнопке — у ▦ и ≡ разная ширина. */
.am-look__btn {
  display: grid;
  place-items: center;
  width: 36px;
...
/* Закладки лентой: шесть чипов с числами на узком окне раскладывались в три этажа и уводили сетку за экран. */
.am-lists__tabs {
  display: flex;
  gap: 8px;
  padding-bottom: 2px;
...
/* Заглушка строки повторяет её высоту: иначе экран подпрыгивал в момент, когда заглушки сменялись живыми строками. */
