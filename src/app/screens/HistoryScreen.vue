...
/** Рельс крестика удаления на пульте: крестик сидит поверх строки, и вертикальный обход
 * ловил его на каждом шаге. Слушатель на окне в фазе перехвата: spatial nav стартует после bubble-фазы keydown. */
...
/** Сколько строк рисуется за раз. История копится сотнями, а каждая строка просит обложку:
 * сразу всё — это сотни картинок в разметке и сотни запросов за названиями. */
const PAGE_LIMIT = 80
...
/** Час просмотра по местным часам. Не через toLocaleTimeString: тот отдаёт двенадцатичасовой
 * вид и точку вместо двоеточия на чужих настройках, а рядом с «12:40 / 24:00» это был бы разнобой. */
...
/** Подпись дня. Сегодня и вчера называются словами: «12 сентября» рядом с ними читалось бы
 * календарём, а не памятью. Год появляется только у чужого года. */
...
/** Раскладывает записи по дням. Список уже отсортирован свежим вперёд, поэтому день у строки
 * либо тот же, что у прошлой, либо следующий: искать по всем дням незачем, хватит последнего. */
...
/** Продолжить с того места, где остановились. Перед переходом запись становится нынешним
 * выбором тайтла: плеер открывается по последнему выбору, и без этого он открыл бы другую серию. */
...
/** Очистка в два нажатия. Отменить её будет нечем, а кнопка стоит в одном клике от
 * «Продолжить»: случайное нажатие не должно стирать полгода просмотров. */
...
/** Русские имена для тех записей, где снимок остался латиницей. Пачками, как в списках:
 * имя — не то, ради чего стоит держать экран пустым, поэтому добор идёт фоном. */
...
<style scoped>
/* Полоса над списком — то же стекло, что у отбора в списках: история стоит в меню рядом с «Моё». */
.am-hist__top {
  display: flex;
  flex-wrap: wrap;
  gap: 10px;
...
/* Взведённая очистка краснеет: второе нажатие необратимо, и это должно быть видно до него. */
.am-hist__danger {
  color: var(--am-bad);
...
/* Заглушка повторяет высоту строки с постером: иначе экран подпрыгивал бы при смене заглушек живыми строками. */
...
/* Цель нажатия — вся строка: целиться в название с «Продолжить» на другом конце строки невозможно. */
.am-hist__hit {
  position: relative;
  display: flex;
  gap: 12px;
...
/* Отступ под крестик держится всегда, а не только под курсором: иначе «Продолжить» дёргалось бы при каждом наведении. */
...
/* Название в одну строку с отсечкой: ровный шаг строк важнее полного имени у трёх самых длинных названий. */
.am-hist__name {
  overflow: hidden;
  font-size: 13.5px;
  font-weight: 600;
...
/* Крестик лежит поверх правого края строки и просыпается под курсором. Полностью прятать
   нельзя: с клавиатуры до него надо доезжать, поэтому гаснет он прозрачностью, а не display: none. */
.am-hist__drop {
  position: absolute;
  top: calc(50% - 4px);
  right: 9px;
