 ...
 *
 * Зоны «плашка продолжения», «пропустить» и «переспросить» стоят на одной
 * кнопке каждая: они появляются и исчезают посреди серии, и без своей зоны
 * кнопка была бы недостижима стрелкой вовсе. Порядок сверху вниз: шапка,
 * плашка над кадром, пропуск, полоса времени, ряд кнопок, отказ, списки.
 *
 * ПОЛОСА ВРЕМЕНИ ОТДЕЛЬНА ОТ РЯДА КНОПОК
 * Ряд кнопок стоит под полосой, и в одной зоне с ней он был недостижим:
 * стрелка вниз внутри зоны — ход поперёк её оси, а он переводит в соседнюю
 * зону, то есть проскакивает ряд целиком. Разделение даёт ровный ход
 * вниз: полоса → кнопки → списки.
 */
const ZONES: Zone[] = [
  { name: 'head', along: 'row' },
 ...
 *
 * Проверка обязательна: панель уезжает с кадра и прячется под заслонкой, а
 * `querySelectorAll` видит кнопки и там, и там. Без неё `focus()` молча не
 * срабатывал, фокус оставался ни на ком, и пульт не попадал на панель
 * вообще — ровно в тот момент, когда человек за ней и потянулся.
 *
 * Проверка размера — про схлопнутые ползунки. Свёрнутая полоса громкости
 * имеет ширину ноль и прозрачна, но видимости по CSS это не мешает: фокус
 * на неё садился, и стрелка застревала на том, чего человек не видит.
 */
...
function itemsOf(root: ParentNode, name: string): HTMLElement[] {
  ...
  // Сама зона тоже годится: плашка и «пропустить» — одиночные кнопки,
  // и обёртку ради одной кнопки городить незачем.
  const own = box instanceof HTMLElement && box.matches(ITEMS) ? [box] : []

  // Только свои, без вложенных зон. Панель обнимает и полосу времени, и
  // ряд кнопок, а полоса — своя зона: без этой проверки шаг «вниз» с
  // полосы попадал в ту же полосу, потому что она же первая кнопка
  // панели. Чужое отсекаем по ближайшей зоне: у своей она совпадает
  // с самой панелью.
...
/**
 * Держит фокус внутри открытого меню: скорости, качества, громкости.
 *
 * Список лежит отдельно от ряда кнопок — над своей кнопкой, поверх кадра, —
 * и в обход рядов не входит: шаг вверх уводил на полосу времени, а до строк
 * списка с пульта не дойти было вовсе. Пока меню открыто, стрелки ходят
 * только по его строкам и наружу не выходят: закрывается меню выбором строки
 * (оно закрывается само) или кнопкой «назад» (её разбирает экран).
 */
function holdMenu(root: ParentNode, here: HTMLElement | null, intent: PlayerIntent): boolean {
  ...
  // Стоим не в меню — встаём на выбранную строку, а не на первую: выбор
  // человек уже сделал, и показывать надо именно его.
...
export function moveFocus(root: ParentNode, intent: PlayerIntent): boolean {
  ...
  // Панели может не быть — под заслонкой она спрятана, — тогда подходит
  // любая зона, в которой есть живая кнопка: хотя бы «Переспросить».
  if (zone === undefined) {
    ...
    // Нажатие всё равно наше: прокручивать здесь нечего.
    return true
  }
  ...
  // Вдоль оси — шаг по зоне. У края (или в зоне из одной кнопки, как шапка)
  // шаг не пропадает, а переводит в соседнюю зону: кнопка, после которой
  // стрелка не делает ровно ничего, читается как поломка пульта.
  //
  // Шаг считаем только от настоящего номера. Фокус может стоять на кнопке,
  // которой в списке нет (свёрнутый ползунок, спрятанная под заслонкой
  // панель): тогда «номер + 1» давал бы первый элемент зоны, то есть прыжок
  // в её начало, а не шаг в сторону.
