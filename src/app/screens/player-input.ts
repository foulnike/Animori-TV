...
/**
 * Порядок зон: вдоль своей оси стрелка водит фокус внутри зоны, поперёк — переводит в соседнюю.
 * Геометрию не угадываем: список задан руками, зато поведение не зависит ни от ширины окна, ни от порядка блоков.
 * У «плашки продолжения», «пропустить» и «переспросить» своя зона: они появляются посреди серии.
 */
const ZONES: Zone[] = [
  { name: 'head', along: 'row' },
