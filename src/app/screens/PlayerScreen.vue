...
import { attachCast, type Cast } from './player-cast'
...
/**
 * Постер кадра на время загрузки.
 *
 * У некоторых площадок первый сегмент HLS — это просто значок «Play», и
 * человек, увидев его на весь кадр, решает, что плеер сломан. Тег
 * `<video>` показывает атрибут `poster`, пока у него нет настоящего
 * первого кадра, и подменяет его сразу, как только кадр приходит.
 * Постер — тёмный фон и сакура: тот же знак, что крутится на заслонке,
 * и человек видит бренд, а не чужую заглушку.
 *
 * Размеры совпадают с рамкой кадра в широком режиме (16:9, 960×540),
 * чтобы сакура в пикселях была того же размера, что и на экране:
 * браузер не масштабирует постер, иначе знак вырастал бы на весь кадр.
 * Геометрия — общая с SakuraMark (sakura.ts), чтобы знак в постере и
 * знак на кнопках был одним и тем же цветком.
 */
const videoPoster = (() => {
  const W = 960
  const H = 540
...
async function wantWindowWide(next: boolean): Promise<void> {
  // На телевизоре окно оболочки и так во весь экран, а мост отвечает
  // отказом. Спрашивать его — значит копить предупреждения в журнале
  // на каждом открытии плеера.
  if (lite) {
    windowWide = next
    return
  }
...
onMounted(() => {
  ...
  if (el !== null) {
    ...
    // Трансляции здесь нет: приставка и есть устройство, на котором смотрят,
    // и зеркалить её экран некуда. От player-cast осталось окошко.
    cast = attachCast(el, {
      onPip: (on) => {
        pipOn.value = on
      },
