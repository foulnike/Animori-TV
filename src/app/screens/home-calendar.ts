// Календарь выхода на Главной: отвечает на вопрос «что выйдет завтра».
//
// Неделя календарная (Пн…Вс, по местным суткам), прошедшие дни остаются.
// День — полка постеров с боковой прокруткой, а не столбец строк: так высота
// не зависит от числа выходов. Номера MAL добирает `fetchMalIds`; метку
// доступности ставит один Kodik (он входит по номеру MAL). Областей две:
// «Моё» (список без брошенного) и «Популярное» (верхушка идущих — AniList
// не умеет «всё, что выходит в мире»). 18+ отсекается на входе, по `isAdult`
// из расписания: иначе скрытый выход тянул бы имя, обложку и вопрос о
// доступности ради строки, которой человек не увидит.
