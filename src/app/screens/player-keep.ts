...
// и что смотрели. Отдельно от core/settings: метка пишется сама всю
// серию, и неудачная запись в общем ключе уносила бы и настройки.
// Одна запись на всё, а не ключ на серию: каждое обращение к
// хранилищу идёт через мост. Громкость и скорость сюда не попадают —
// вернувшаяся через неделю двойная скорость читается поломкой.
//
// История — не метки: метка отвечает «с какой секунды продолжать» и
// умирает по досмотру, история отвечает «что смотрели, когда и сколько».
// Ключ истории склеен из аниме и серии без озвучки: три дорожки одной
// серии стояли бы тремя строками об одном. Заголовок и обложка едут
// вместе с записью — экран истории рисуется до всякой сети.
