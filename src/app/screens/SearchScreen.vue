 ...
 * буквами: «наруто» уезжало тремя-четырьмя запросами. Половина секунды ловит слово целиком. */
...
/** СЧЁТЧИК ГОВОРИТ О ПОКАЗАННОМ: точное число приезжает только с последней страницы.
 * Пока страницы не кончились, у AniList вместо подсчёта оценка с потолком, и слой сети
 * отдаёт пустоту — над двумя дюжинами постеров стояло «5000». Тогда счётчик считает по
 * своей выдаче и честно добавляет плюс: «27 и ещё», а не выдумывает каталог целиком. */
...
// Ответы про доступность приходят по одному и вразброд, а выдача уже на экране:
// без подписки метки появлялись бы одним рывком в конце захода.
 ...
 * спрашивается вся выдача разом: однажды спрошенное показывается целиком и даром. */
...
/** Спрашивает каталог. С `add` добирает следующую страницу, без него начинает с
 * первой; устаревшие ответы отбрасываются по номеру работы. `force` — человек нажал
 * Enter: спрашиваем набранное как есть, не глядя ни на порог длины, ни на повтор. */
async function search(add = false, force = false): Promise<void> {
  ...
  if (!add && !force) {
// Слово не изменилось: выдача по нему уже на экране — сюда приходят возвраты каретки и смена раскладки.
    if (wordNow === lastAsked) return

// Одну-две буквы каталог понимает как «отдай что угодно»: такую страницу не читают, а стоит она полного запроса.
  ...
  try {
    ...
    if (found === null) {
// Отказ не считается заданным вопросом: иначе повтор того же слова упирался бы в защиту от повтора.
      lastAsked = ''
...
// Сеть про доступность отсюда не спрашивается: подъём со склада бесплатный, а вопросы службам закажут сами плитки.
  priming = loadMarks()
}
...
// Пустому полю ждать нечего: выдачу надо снять сразу, заодно снимается ответ по стёртому слову.
  if (asked.value === '') {
    timer = null
    drop()
    return
...
onMounted(() => {
  void (async () => {
// Поиск бывает первым экраном запуска: без подъёма снимка память пуста, и своих меток на постерах не будет.
    try {
...
<!-- v-seen сообщает о первом показе плитки: только о показанных спрашиваются источники видео (app/see-tile.ts). -->
...
<style scoped>
/* Поле поиска — главный предмет экрана, а не одна из контролок в ряду. Обёртка — label: клик по капсуле ставит курсор в поле. */
.am-hunt {
  display: flex;
  gap: 12px;
  align-items: center;
...
/* Кромка вместо обводки поверх капсулы: два кольца одно в другом выглядели браком рисования. */
...
/* Крестик очистки у type=search рисуется тёмным квадратом на светлой теме: приводим его к цвету текста. */
.am-hunt__field::-webkit-search-cancel-button {
  cursor: pointer;
  filter: grayscale(1) opacity(0.6);
}

/* Счётчик внутри поля: отдельная строка ради одного числа сдвигала всю выдачу вниз. */
.am-hunt__num {
  flex: 0 0 auto;
  padding: 3px 10px;
  font-size: 12px;
...
/* Заглушка — элемент сетки, а общий .am-hold в слое тем сам сетка: внутри .am-grid его надо вернуть в колонку. */
.am-hold {
  display: flex;
