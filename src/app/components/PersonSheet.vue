...
// «Назад» закрывает карточку человека и возвращает к тому, откуда её
// открыли, — а не уводит по истории на прежний экран. Под окном остаётся
// карточка тайтла, и её человек как раз и читал.
      ...
      <div ref="box" class="am-sheet__body">
        ...
        <template v-else>
          ...
          >
            ...
            <!-- Описание человека сплошным текстом: ссылки в нём ведут на
                 других персонажей и авторов, а человек, читающий биографию,
                 идёт за ними не собираясь — и уходит из карточки в цепочку
                 чужих биографий. -->
...
/* ТЕЛЕВИЗОР
   Окно человека рассчитано на мышь и на то, что закрыть его можно
   крестом в углу или Escape. Ни того, ни другого у пульта нет, а
   стрелки из окна уводили фокус на страницу под ним (причина — в
   dpad: окно стоит на position: fixed, и область видимости окна
   не определялась). Здесь только размер: окно шире и ниже, портрет
   и постеры работ мельче, описание не растягивается. */
...
/* Крест закрытия на телевизоре — главный выход из окна, и 44 пикселя
   для него мало: цель должна ловиться с трёх метров. */
