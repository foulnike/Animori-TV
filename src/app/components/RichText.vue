...
// Описание разметкой источника: начертания, перекрёстные ссылки, спойлеры, цитаты и списки.
// Разбор живёт в core/rich-text.ts, здесь только показ. Компонент зовёт себя сам для вложенного уровня.
...
/** Начертания куска: классы вместо стилей — вид решает тема. */
...
/** Заказ имён по всему описанию. Сторожем, а не из разметки: вызов с побочным
 * действием прямо в рисовании заводит слежение по кругу. */
...
/** Идёт по ссылке из описания. Окошко человека закрывается только на переходе к тайтлу:
 * после ссылки на другого человека окошко уже показывает нового, закрывать нечего. */
...
<style scoped>
/* Зазор между блоками держит раскладка: у абзацев маргины сняты, иначе спойлер и цитата вставали то вплотную, то с двойным отступом. */
.am-rich {
  display: flex;
...
/* Переносы источника сохраняются: описания набирают строками, склейка в одну простыню теряет и стихи, и перечисления без тегов. */
.am-rich__para {
  margin: 0;
...
/* Ссылка внутрь и наружу выглядит одинаково: разница видна по тому, куда она привела, а два вида подчёркивания в абзаце только шумят. */
.am-rich__link {
  color: var(--am-text);
  text-decoration: underline;
  text-underline-offset: 2px;
...
/* Значок раскрытия стоит в своей клетке: иначе строка дёргалась при смене треугольника. */
.am-rich__mark {
  display: grid;
  place-items: center;
  width: 10px;
...
/* Картинка из описания вписывается в колонку: у источника они бывают в полный размер экрана. */
.am-rich__art {
  display: block;
  max-width: 100%;
  height: auto;
