...
// Пункт 3.9а: окно правки записи списка. Держит только черновик; хранение
// и сеть — дело памяти списка.
//
// Окно телепортируется в body: над списком есть предки с трансформом и
// размытием, они забирают отсчёт у fixed и уносят окно за край видимого.
// Правки идут черновиком, наружу уходит только «Готово» — иначе «не сохранять»
// на крестике было бы неправдой. Дойдя до потолка серий, закладка сама
// становится «Просмотрено» (онгоингу — нет: потолок там число вышедших
// серий, а не итог). «Готово» на миг отвечает «Сохранено»: по одной
// исчезнувшей шторке не понять, записалось ли.
