...
// Инструкция: как перенести список на телевизор — один путь (компьютер делает ссылку, приставка забирает копию).
// Отдельной модалкой, а не абзацами в панели: читают её один раз; своего состояния узел не держит вовсе.
// Уводится в body: position: fixed внутри предка с transform считается от предка и уезжал бы с прокруткой.
