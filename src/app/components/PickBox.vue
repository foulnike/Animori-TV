...
/**
 * Стрелка на закрытом списке не делает ничего.
 *
 * Раньше любая стрелка его раскрывала — так ведёт себя нативный
 * `<select>`, и на компьютере это удобно. На телевизоре стрелку
 * разбирает пульт (dpad.ts), и на одно нажатие выходило два действия:
 * список разворачивался, и фокус уезжал в него. Замер с приставки:
 * проход вниз по панели прокси останавливался на пунктах «HTTP»
 * и «SOCKS5» вместо того, чтобы идти дальше по настройкам.
 *
 * Открывает список Enter — как и всякая другая кнопка на пульте.
 */
