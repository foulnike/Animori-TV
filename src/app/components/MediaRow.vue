...
// Отдельный компонент, а не режим MediaTile: плитка стоит на постере во всю колонку и держит метки поверх обложки.
...
<template>
  ...
  >
    ...
    <!-- Знак ползунков, а не карандаш: правятся параметры записи —
         ...
         `data-am-beside` говорит обходу пульта, что кнопка — спутник
         строки: к ней ходят вправо, а вверх и вниз водят по самим строкам.
         Без метки шаг вниз сходил на неё, а не на следующую строку: у строки
         с полосой пройденного она поднята на четыре пикселя и по метрике
         оказывалась ближе соседней строки ровно на эти четыре. Замер
         с приставки: до следующей строки человек добирался вторым нажатием. -->
    <button
      v-if="editable"
