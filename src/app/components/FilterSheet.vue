...
/** Выбранные тэги отдельной строкой: тэг из закрытой группы иначе не виден, и снять его было бы нечем. */
const chosenTags = computed<string[]>(() => draft.value.tags)

/** Поднимает справочник тэгов. Провал виден пустым разделом: остальные разделы работают. */
...
/** Поля годов читаются сырыми: подгонка под границы на каждую цифру не давала бы ввести «1995». */
function onYears(): void {
  draft.value.yearFrom = readYear(fromText.value)
  draft.value.yearTo = readYear(tillText.value)
}

/** Ввод в поле года: всё, кроме цифр, снимается, длина держится в четырёх знаках.
 * Значение возвращается в сам узел — иначе показ отстанет от состояния при чистке. */
...
/** Шаг стрелкой на год. Пустое или недописанное поле сперва встаёт на нынешний год:
 * иначе стрелка вверх превратила бы набранное «19» в «20». */
...
/** Сброс чистит черновик, но не закрывает меню: чаще сбрасывают, чтобы собрать другой отбор. */
...
/** Отдаёт отбор наружу. Годы приводятся к границам здесь, а перевёрнутый промежуток
 * меняется местами: «с 2015 по 2005» — описка, а не просьба показать пустоту. */
...
// При открытии берётся свежая копия отбора: брошенный черновик не должен вскрываться снова.
watch(
  () => props.open,
  (open) => {
    if (open) {
...
<style scoped>
/* Меню поверх всего: витрина под ним остаётся на месте, чтобы возврат не сбрасывал прокрутку главной. */
.am-sheet {
  position: fixed;
  inset: 0;
  z-index: 60;
...
/* Шапка и низ держатся на месте, ездит только середина: иначе «Готово» уезжает за край при длинном справочнике. */
.am-sheet__top,
.am-sheet__foot {
  display: flex;
  flex: 0 0 auto;
...
/* Поле поиска в колонке тянется на всю ширину: у общего .am-search расчёт на ряд контролок. */
.am-part__hunt {
  flex: 0 0 auto;
  width: 100%;
}
...
/* Стрелка раскрытия в своём квадрате: у рисунка нет базовой линии, и поворот идёт вокруг его центра. */
.am-fold__arrow {
  display: grid;
  flex: 0 0 auto;
  place-items: center;
...
/* На узком экране меню становится нижним листом: коробка по центру оставляла бы полосы пустоты сверху и снизу. */
