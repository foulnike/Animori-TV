...
// Плитка аниме: постер, название и метки. Одна на списки, поиск и полки; своих данных не добывает.
// Наведение — на всей плитке, не на кнопке открытия: иначе переход на крестик читался уходом с плитки.
...
/** Подсказка отметки повторов. Сам счёт живёт только здесь: цифра рядом со знаком
 * читалась в сетке как вторая оценка. */
const repeatHint = computed(() => `Повторных проходов: ${props.repeat}`)

/** Метка доступности. У анонса её нет: смотреть нечего по природе аниме, и об этом уже сказано меткой анонса.
 * Сидит в правом нижнем углу, а не среди меток слева: доступность — про наш плеер, её место рядом с кнопкой просмотра. */
...
<template>
  <li class="am-tile" :class="{ 'am-tile--hidable': hidable, 'am-tile--edit': editable }">
    ...
    <!-- Знак «✕» спрятан от чтения с экрана, а v-tip рисуется своей плашкой в body и имени кнопке не даёт — отсюда явный aria-label. Подложка — цветок: см. SakuraBloom.vue. -->
    <button
      v-if="hidable"
      v-tip="'Не интересует'"
      class="am-tile__hide"
    ...
    <!-- Знак ползунков, а не карандаш: правятся параметры записи, а не текст. Карандаш занят знаком своего комментария в метках слева. -->
    <button
      v-if="editable"
...
<style scoped>
/* Цвет текста и меток поверх постера не тематический: завеса под ними тёмная во всех трёх темах.
   --am-art-deep — то же, но плотное: для фигур, которые накладываются сами на себя и от полупрозрачности пошли бы швами. */
.am-tile {
  --am-on-art: #eef3fb;
  --am-art-deep: #0b1017;
...
/* Форма «листа»: три угла круглые, один почти острый. Ряд таких плиток читается ритмом, а не сеткой одинаковых прямоугольников. */
.am-tile__art {
  position: relative;
  display: block;
  aspect-ratio: 2 / 3;
...
/* Блик пробегает по постеру один раз на наведение: постоянное свечение в сетке на сотню плиток было бы рябью. */
.am-tile__sheen {
  position: absolute;
  inset: -40% -20%;
  background: linear-gradient(
...
/* Метки — стекло, а не плотные пилюли: на светлых обложках чёрные плашки читались как грязь на картинке. */
.am-tile__tag {
  display: inline-flex;
  align-items: center;
  padding: 3px 9px;
...
/* Метка анонса: единственная цветная в ряду, и только потому, что говорит об аниме главное: смотреть пока нечего. */
.am-tile__tag--soon {
  color: var(--am-on-art);
...
/* Знаки пересмотра и заметки — круглые монетки без подписей: ряд пилюль разной длины читался как список оценок. Счёт и текст — в подсказке. */
.am-tile__tag--sign {
  display: grid;
  place-items: center;
  width: 22px;
...
/* Оценка каталога: стекло с тёплым текстом. Заливка жёлтым кричала громче самого постера. */
.am-tile__score {
  position: absolute;
  top: 8px;
  right: 8px;
...
/* Счёт частей и метка доступности стоят в одной строке, поэтому счёт уступает ей полосу справа: иначе «12 / 24» уезжало бы под знак. */
.am-tile__own--play {
  right: 28px;
}
...
/* Сам знак. clip-path, а не рамки: так треугольник остаётся ровно в центре своей клетки и поверх него можно положить перечёркивание.
   Полпикселя вправо — правка оптическая: у треугольника центр тяжести левее середины. */
...
/* «Нет в каталоге»: тот же знак, только погашенный и перечёркнутый. Это отсутствие в нашем плеере, а не свойство аниме:
   красным здесь кричать не о чем, и в ряду плиток метка обязана молчать — оттого у неё нет ни ореола, ни роста под курсором. */
.am-tile__play--none {
...
/* Черта идёт по диагонали от края до края: короткий штрих поверх треугольника читался царапиной.
   Тёмная обводка отделяет её от знака под ней: без оправы два светлых слоя одного цвета слиплись бы в пятно. */
...
/* Символ лежит поверх цветка: цветок вынут из потока через absolute, а позиционированное рисуется выше обычного —
   без своего z-index знак ушёл бы под лепестки. Центровку держит сетка родителя. */
.am-tile__hide-sign {
  position: relative;
  z-index: 1;
  display: block;
...
/* Правка записи собрана тем же способом, что крестик витрины, только в другом углу и с другой точкой роста. Всё прочее — отсутствие формы, цветок под знаком, расширенная цель нажатия — взято оттуда же. */
.am-tile__edit {
  --am-bloom-deep: var(--am-art-deep);
  --am-bloom-petal: color-mix(in srgb, var(--am-sakura) 34%, var(--am-art-deep));
  --am-bloom-out: 2px;
...
/* Знак поверх цветка — та же причина, что у крестика. Линии рисуются цветом текста, поэтому оттенки знака живут в одном правиле с кнопкой. */
.am-tile__edit-sign {
  position: relative;
  z-index: 1;
  display: block;
...
/* Где курсора нет вовсе — правка видна сразу: навести пальцем нельзя, а без кнопки правка со списка была бы недоступна.
   Крестик витрины сюда не входит нарочно: «не интересует» — действие редкое и необратимое, и постоянный крестик ловился бы пальцем случайно. */
@media (hover: none) {
  .am-tile__edit {
    visibility: visible;
    opacity: 1;
