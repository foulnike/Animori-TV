...
// На приставке нет ни окна выбора папки, ни буфера обмена: кнопки, которые заведомо ничего не сделают, не рисуются.
...
/** Расширения, которые примет оболочка (тот же список в export.rs). Незнакомое
 * считаем ogg: AnimeThemes раздаёт именно ogg. */
...
/** Громкость живёт вне компонента: карточка пересобирается при каждом переходе,
 * и выставленный уровень иначе возвращался бы к своему значению на каждом тайтле. */
let keepVol = 0.8
...
/** Перемешивание: автоматически следующая тема берётся случайно, а не по порядку.
 * На кнопки шага оно не влияет — почему, сказано в stepBy. */
const shuffle = ref(false)

/** Тянут ручку таймлайна: показания времени в это время наши, не плеера. */
const drag = ref(false)

/** Открыто ли окно тем. Пульт и список живут там, а не в полосе: полосе досталась
 * одна строка, и всё остальное уехало в окно за кнопкой. */
...
/** Следующая тема со звуком. По порядку — ближайшая ниже заряженной, с
 * перемешиванием — случайная из остальных: иначе список ходил бы одним кругом. */
...
/** Доводит звучащую строку в окно списка: видно шесть строк, а тема сменяется и
 * сама. Ближним краем, а не серединой — доводка показывает строку, не перетряхивая окно. */
...
/** Открывает окно тем и доводит звучащую строку в видимую часть. Доводка ждёт
 * появления списка в разметке: у закрытого окна списка нет, и ссылка пуста. */
...
/** Клавиши внутри блока: пробел — пуск и пауза, стрелки — перемотка. Пробел на
 * кнопках отдаём кнопке: там он и так нажатие. */
...
/** Готовая ссылка темы на службу или null. onlyMusic нужен Ютубу: под ключом youtube
 * служба держит и обычные ролики, а кнопка YouTube Music обещает именно Музыку. */
...
/** Расширение из адреса без запроса и якоря. Незнакомое — считаем ogg. Обрезка
 * поиском разделителя, а не split по индексу: при noUncheckedIndexedAccess элемент пуст. */
...
/** Название с автором в буфер: готовая строка для поиска на стриминге, когда
 * кнопки поиска мало и подпись несут куда-то ещё. */
...
/** Загрузка одной темы файлом. Порядок намеренно такой: сперва папка, потом сеть —
 * скачать мегабайты и потом узнать, что окно выбора закрыли, значило бы тратить канал.
 * Запрос идёт через мост, а не через fetch окна: байты нужны оболочке. Ограничитель
 * AnimeThemes сюда не замешан нарочно: он стережёт АПИ, а звук раздаёт отдельная
 * раздача. Ошибки тихие: причина уходит в журнал. */
...
// Ключ обязан быть уникален, даже когда номера совпали: две разные песни под одним
// номером случаются. С одинаковыми ключами Vue путает строки при обновлении, и в
// списке появляется одна и та же песня дважды. Подпись от номера не пострадает.
    const out: TuneRow[] = []
...
<style scoped>
/* Токены роста списка. Объявлены и на полосе, и на окне: окно уезжает в body, общего
...
/* ПОЛОСА. Одна строка над нижней кромкой окна: сверху органы и подпись, снизу таймлайн.
   Липнет к низу — под ней уже оставлено поле в .am-view, поэтому плитки она не накрывает.
...
/* Верхний ряд полосы: шаг, пуск, тема, время, кнопка списка. Высота задана, а не набрана
   содержимым: по ней же посчитан рост полосы. Имя с __top: __row занято кнопкой строки списка. */
.am-tune__top {
  display: flex;
  gap: 10px;
  align-items: center;
...
/* ПУСК В ПОЛОСЕ МЕНЬШЕ, ЧЕМ В ПУЛЬТЕ: сорок шесть пикселей задавали бы рост всей полосы. */
...
/* Шаг по темам — круглыш того же роста, что кнопка списка: органы одной полосы должны быть одного размера. */
.am-tune__step,
.am-tune__open {
  display: grid;
  flex: none;
...
/* Кнопка списка шире прочих: в ней кнопка и счётчик тем одним органом, нажатие одно. */
.am-tune__open {
...
/* Подпись звучащего занимает всю середину полосы и прижата влево: центровка осталась пульту. */
.am-tune .am-tune__now {
  flex: 1;
  justify-content: flex-start;
}

/* Таймлайн нижней кромкой: своя высота, растягиваться ему некуда — рост по главной оси был бы вертикальным. */
.am-tune .am-tune__seek {
  flex: none;
  height: 14px;
}

/* Знаки полосы мельче, чем в пульте: девятнадцать пикселей на кнопке в двадцать шесть — знак без воздуха. */
.am-tune .am-tune__glyph {
  width: 15px;
  height: 15px;
}

/* Счётчик тем бледной пилюлей: цифра рядом с заголовком заменяет подпись «всего тем». */
.am-tune__count {
  padding: 2px 8px;
  font-size: 11px;
  font-weight: 650;
...
/* ОКНО ТЕМ. Тот же приём, что у подбора и записи в список: затемнение, коробка по центру,
   шапка на месте, ездит только середина. Имена классов общие (.am-sheet), а правила свои:
   у каждого окна свой scoped-блок. Ширина 760: строка темы — знак, номер, название,
   исполнитель и пять кнопок, и в узкой коробке они не помещаются в одну строку. */
.am-sheet {
  position: fixed;
  inset: 0;
  z-index: 60;
...
/* Шапка держится на месте: список тем длинный, и уехавший заголовок с кнопкой заставлял бы искать их прокруткой. */
.am-sheet__top {
  display: flex;
  flex: 0 0 auto;
  gap: 10px;
...
/* Центр держит сетка, а не подобранные отступы: крайние колонки равные, и цветок в
   средней стоит по середине панели при любой ширине боковых групп. Ряд — контейнер для
   запроса по ширине: у ряда нет отступов и рамки, и порог считается по чистой ширине. */
.am-tune__organs {
...
/* Кнопка остаётся прямоугольной и без своей одежды: круг и сакуру рисует вложенный слой,
   а кнопке остаются попадание по всей цели и кольцо фокуса. Оттенки цветка — как в окне правки. */
.am-tune__hit {
...
/* Играет — цветок остаётся распущенным сам и живёт: лепестки крутятся, сердцевина дышит. */
.am-tune__hit--live {
  color: var(--am-text);
}
...
/* Подпись звучащего под полосой и по центру: заголовок пульта, а не строка слева. */
.am-tune__now {
  display: flex;
  gap: 7px;
...
/* Цель тяги высокая, а полоса тонкая: ручку в четыре пикселя мышью не поймать, под ней прозрачный запас. */
.am-tune__seek,
.am-tune__vol {
  position: relative;
  display: flex;
...
/* Громкость короче таймлайна, но не огрызок: на шестидесяти пикселях одно деление шло
   шесть процентов. СЖИМАЕТСЯ, А НЕ ВЫЛЕЗАЕТ: прежде flex: none держал 96 пикселей, и в
   узком окне ползунок выезжал за панель. Теперь база 96 и разрешение сжаться — отдаёт
   ширину первым, потому что кнопка звука важнее. */
.am-tune__vol {
  flex: 0 1 96px;
  min-width: 0;
}

/* ПРЕДОХРАНИТЕЛЬ. Ниже 48 пикселей ползунок перестаёт быть органом, и там он убирается
   вовсе — но не звук: кнопка рядом остаётся. ПОЧЕМУ ЗАПРОС ПО КОНТЕЙНЕРУ, А НЕ ПО ОКНУ:
   ширина панели зависит от раскладки карточки, а не от окна, и медиазапрос срабатывал бы
   вразнобой. container-type: inline-size стоит на ряде органов. ПОРОГ 340: ряд — две равные
   колонки, цветок с шагом (130) и два просвета по 18; на ползунок остаётся ровно 47. */
...
/* Середина пульта: шаг по списку и пуск одним рядом. Зазор уже, чем между колонками
   сетки: шаг и пуск — один орган, и зазор в 18 пикселей разорвал бы его натрое. */
.am-tune__mid {
  display: flex;
  gap: 8px;
  align-items: center;
...
/* Боковые группы занимают свою колонку целиком, а к цветку прижимают содержимое
   (justify-content), не себя. При justify-self ширина группы мерилась по содержимому:
   ползунок обязан был занимать свои 96 пикселей всегда, и всё лишнее выезжало за панель. */
.am-tune__tools {
  display: flex;
  gap: 6px;
  align-items: center;
...
/* Органы пульта размером под палец: рядом с цветком в 46px кнопки в 28 читались мелочью. */
.am-tune__tool {
  display: grid;
  flex: none;
  place-items: center;
...
/* Шаг выключен, когда тема со звуком одна: шагать некуда, и кнопка говорит об этом. */
...
/* Включённый орган светится акцентом: без этого состояние кнопки приходилось бы проверять на слух. */
.am-tune__tool--on {
  color: var(--am-accent);
...
/* Заливкой, а не обводкой: треугольник шага — сплошной знак, и обведённый на девятнадцати
   пикселях читается пустой скобкой. Обводка снята целиком: полоса записана прямоугольником. */
.am-tune__glyph--fill {
  fill: currentcolor;
  stroke: none;
}

/* Окно списка ростом ровно в шесть строк: седьмая и дальше достаются прокруткой. Просветы
   в счёте участвуют — без них снизу выглядывала бы полоска седьмой строки. Прокрутка не
   уходит на страницу: докрутив список, человек терял бы строки из вида. */
.am-tune__list {
  display: flex;
  flex-direction: column;
  gap: var(--am-tune-gap);
...
/* Пункт списка и есть строка: отступы, подсветка наведения и рамка выбора висят здесь,
   поэтому кнопки стримингов читаются её частью. Рост задан, а не набран содержимым: на нём
   стоит обещание «видно ровно шесть». Сжиматься строке запрещено — иначе flex сдавил бы
   восемь строк по окну вместо того, чтобы отдать их прокрутке. */
.am-tune__item {
  display: flex;
  flex: none;
  gap: 3px;
...
/* Вся строка — цель нажатия: выбор темы мышью не должен требовать попадания в круглыш. */
.am-tune__row {
  display: flex;
  flex: 1;
  gap: 9px;
...
/* Темы без записи встречаются: строка остаётся в списке со ссылками и подписью, но не зовёт нажать. */
.am-tune__row--mute {
  cursor: default;
  opacity: 0.55;
}

/* Кнопки строки показываются под курсором и при фокусе с клавиатуры. Прячется прозрачность,
   а не сама кнопка: иначе названия прыгали бы, а скрытая кнопка не могла бы поймать фокус. */
.am-tune__tunes,
.am-tune__acts {
  display: flex;
  flex: none;
...
/* Стриминги строки: три круглых знака перед кнопками копирования и загрузки. Знак цветной,
   поэтому кнопка под ним прозрачная, а подсветка наведения — бледное кольцо. */
.am-tune__jump {
  display: grid;
  flex: none;
  place-items: center;
...
/* Отметка о сделанном акцентом и на пару секунд: уведомление на полэкрана ради одной строки было бы перебором. */
.am-tune__act--done {
  color: var(--am-accent);
  background: var(--am-accent-soft);
}

/* Ожидание крутит дугу: трек весит мегабайты, и без знака жизни нажатие казалось бы провалившимся. */
.am-tune__act--wait {
  color: var(--am-accent);
}
...
/* Скачивание идёт и без курсора над строкой: отметка о работе не должна исчезать вместе с наведением. */
...
/* Три палочки эквалайзера у звучащей строки: нарисованы полосками и качаются со своим сдвигом каждая. */
.am-tune__beats {
  display: flex;
  gap: 2px;
...
/* Исполнитель через точку и бледнее: это подпись к названию, иначе блок из восьми тем вырастал вдвое. */
.am-tune__artist {
  overflow: hidden;
  font-size: 12px;
  color: var(--am-faint);
...
/* Просьба о покое сильнее красот: цветок просто остаётся распущенным, палочки — поднятыми. */
