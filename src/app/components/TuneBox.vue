...
// Музыка тайтла: темы с AnimeThemes, плеер один на весь блок.
//
// Полоса липнет к низу окна — там уже оставлено поле в 72 px — и не отнимает
// строку у карточки; пульт, список и громкость живут в окне за кнопкой справа.
// Список прокручивается, а не раскрывается (у One Piece двенадцать тем).
// Стримингов три и стоят всегда: готовых адресов служба почти не знает.
// Визуализатор идёт ровным ходом, не в такт: AnalyserNode требует CORS,
// которого у выдачи AnimeThemes нет. Полоса молчит, пока тем нет.
