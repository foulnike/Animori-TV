...
// Копия списка по ссылке. Из шести прежних кнопок на приставке работает
// одна — «забрать по ссылке»: пропуск под шесть десятков знаков пультом
// не набрать, а браузера нет. Ссылку делает компьютер, здесь набирают
// только её хвост после последней косой (ядро примет и то и другое).
//
// Порядок: сперва показать, что нашлось по ссылке, потом предлагать
// положить поверх списка — замена вслепую по строке из пульта это способ
// потерять список из-за одной опечатки. Своим узлом, а не частью настроек:
// наружу одно число записей и одна весть «список сменился».
