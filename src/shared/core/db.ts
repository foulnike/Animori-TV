...
/**
 * Что за запись лежит под префиксом ключа. Таблица, а не череда else if: ветки дважды отставали
 * от кода на целое поколение ключей, и при живом складе счётчик показывал ноль.
 * Сводка склада — мерило того, сколько запросов мы уже не делаем; врущее мерило хуже отсутствующего.
 */
