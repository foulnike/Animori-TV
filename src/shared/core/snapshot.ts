...
/**
 * Номер версии снимка: поднимать при любом изменении формы SnapshotEntry. Миграций нет — старый снимок дешевле выбросить.
 * 2 — метка взрослого; 3 — названия; 4 — тип и тома; 5 — пересмотры, даты, комментарий; 6 — тип и тома убраны (манги нет).
 * Необязательное поле добавляется без поднятия версии: поднятие обесценило бы снимок целиком.
 */
