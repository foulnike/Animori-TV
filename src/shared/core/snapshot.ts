...
/** Имя файла дубля снимка: путь выбирает оболочка (src-tauri/src/files.rs). */
const SNAPSHOT_FILE = 'animori-snapshot.json'

/**
 * Номер версии снимка: поднимать при любом изменении формы SnapshotEntry — миграций нет,
 * старый снимок дешевле выбросить. Необязательное поле добавляется без поднятия версии.
 */
...
export interface SnapshotEntry {
  mediaId: number
  /**
   * Номер MAL или null: по нему тайтл ищется у русских источников. Необязательное поле —
   * снимки до его появления его не имеют, и номер добирается картой датасета или сетью.
   */
  malId?: number | null
  status: string | null
  ...
  score10: number
  progress: number
  /** Счётчик завершённых пересмотров; «Пересматриваю» — отдельное состояние закладки. */
  repeat: number
  /**
   * Даты начала и конца в виде ГГГГ-ММ-ДД. Строка, а не метка времени: это календарный
   * день без часов и пояса, и перевод в миллисекунды сдвигал бы его на сутки.
   */
  ...
  updatedAt: number
  /** Взрослый тайтл. Метка лежит рядом с записью: отбор идёт по всему списку и без сети. */
  isAdult: boolean
  /**
   * Название латиницей и английское. Лежат в снимке, а не на складе: склад расходен
   * и протухает. Русское название здесь не хранится — оно живёт на складе с описанием.
   */
  romaji: string | null
  english: string | null
}
 ...
 * `userId` лежит рядом, иначе после смены входа показали бы чужой список; null — список местный.
 */
...
/** Пустой снимок: отдаётся вместо null, чтобы вызывающий не проверял каждый раз. */
...
/**
 * Приводит прочитанную запись к нынешней форме: файл дубля мог быть правлен руками,
 * а метка взрослого без значения означает «нет». Поля перечислены явно — что не переписано
 * здесь, то на диск и не попадёт.
 */
...
/** Разбирает прочитанное в снимок; битое и стороннее отбрасывается в пустой целиком. */
...
/** Читает снимок с диска; не отклоняется никогда — пустой список лучше мёртвого запуска. */
 ...
 * два пишущих в одну запись теряют правки друг друга.
 */
...
/**
 * Планирует запись снимка. Серия вызовов даёт одну запись: таймер взводится только свободный.
 * Дубль в файл отложенная запись не делает — файл страхует от чистки данных окна.
 */
...
/**
 * Пишет снимок немедленно и дожидается диска. Не отклоняется; отложенная запись отменяется,
 * иначе после ухода в фон будет вторая, пустая.
 *
 * @param options.backup Писать ли второй экземпляр в файл. По умолчанию нет: дубль нужен
 * на точках сохранения и при полной замене списка, а не на каждую правку оценки.
 */
