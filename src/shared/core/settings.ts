...
export interface AniMoriSettings {
  ...
  hideAds: boolean
  /**
   * Показывать ли взрослое (18+) — пункт 3.8. Прячется везде: поиск, каталог,
   * главная, части франшизы, работы человека и студии, календарь и свои списки.
   *
   ...
   *
   * Своего списка ключ касается наравне с остальным, и это пересмотр прежнего
   * решения. Причина пересмотра — не строгость, а числа: счётчики закладок
   * считаются по видимому, а спрятанное называется словами под списком,
   * поэтому прятать больше не значит врать. История просмотров остаётся
   * исключением: это запись о просмотренном, а не каталог (см. core/adult.ts).
   *
   * Включение спрашивает дату рождения (core/adult.ts, `adultByBirth`).
   * Проверка формальная и нарочно не запоминается: хранить чужую дату
   * рождения ради экономии одного нажатия — плохая сделка.
   */
  showAdult: boolean
  /**
