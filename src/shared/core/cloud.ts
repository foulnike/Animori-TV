...
// Облако — хранилище, а не второй хозяин: пишет и читает только по действию человека.
// Чужую правку не затираем молча (сверяем строку времени от облака); по ссылке только читают.
...
/**
 * Прикладывает прочитанную копию к списку: один порядок для обоих путей (своя по пропуску, чужая по ссылке).
 * Чужая копия не сливается: метки времени двух людей между собой ничего не значат, замена — осознанный переезд.
 * Пустая копия не замещает живой список: для стирания есть отдельная кнопка с подтверждением.
 */
