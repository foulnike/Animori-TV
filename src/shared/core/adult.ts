...
/**
 * Показывать ли взрослое прямо сейчас. Читается в момент вопроса:
 * тумблер настроек меняет ответ без перезапуска окна.
 */
...
/**
 * Отсеивает запрещённое из готовой выдачи; признак берётся снаружи, у каждого вида под своим именем.
 * Возвращает тот же массив, когда прятать нечего: лишняя копия сотни находок ничего не даёт.
 */
 ...
 * а честная строка снимает вопрос.
 */
...
/**
 * Сколько полных лет на названную дату; `null` — дата не годится. Считаются именно полные годы:
 * разница годов пропустила бы семнадцатилетнего, которому восемнадцать через месяц.
 * Время передаётся доводом: проверка обязана быть воспроизводимой в тесте.
 */
 ...
 * ошибка здесь пускала бы вперёд, а не назад.
 */
