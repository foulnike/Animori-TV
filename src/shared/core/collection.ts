...
/**
 * Переносит список с сервера в память, по умолчанию слиянием: замена целиком стёрла бы набранное здесь.
 * Зовётся только по прямому действию человека: неожидаемое движение списка читается как поломка.
 * Без входа переносить неоткуда, и это отказ, а не тихий ноль.
 */
...
/**
 * Переносит список с Шикимори по нику: правила те же (слияние; замещение — по прямой просьбе).
 * Хозяин списка не меняется и не сбрасывается: хозяин — счёт AniList, иначе следующий перенос счёл бы список чужим.
