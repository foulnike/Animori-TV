// Клиент Яндекс Диска: только сеть и разбор. Тело возится в два шага по одноразовому адресу.
// Копия лежит в папке приложения (app:, область disk.app_folder): корня Диска пропуск не видит.
// Пропуск приходит аргументом и здесь не хранится; по опубликованной ссылке копию только читают.
