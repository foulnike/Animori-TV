// Поиск персонажей и авторов Shikimori: до трёх REST-запросов, фоллбэк на GraphQL, гард тёзок.
// Состав берём через GraphQL: в REST у персоны нет japanese, а чтения латиницей расходятся.
// Поиск прекращается на балле SURE_SCORE; запасной эндпоинт — только когда первые два промолчали.
