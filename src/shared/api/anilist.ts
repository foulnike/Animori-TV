// Клиент AniList GraphQL: общая пауза по лимиту и разбор ответов; сам запрос собирает мост.
// Тормоз здесь, а не в очереди: только клиент видит все запросы к AniList сразу.
// Отступ переживает перезапуск: иначе новое окно первым же делом шлёт залп в закрытую дверь.
